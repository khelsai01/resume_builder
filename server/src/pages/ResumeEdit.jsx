    ...
    const fetchResume = async () => {
      try {
        const res = await axios.get(
          `https://resume-builder-server-51je.onrender.com/resume/${id}`,
          {
            headers: {
      ...
      const res = await axios.patch(
        ...
        resume,
        {
          headers: {
            ...
            alert("Resume edited successfully! see in Dashboard");
            navigate('/')
  ...
  const handleGenerateSuggestions = async () => {
    try {
      const response = await axios.post(
        "", // Add the appropriate URL for generating suggestions
